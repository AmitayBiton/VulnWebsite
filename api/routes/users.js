...
router.post("/:userId/changePassword", function (req, res) {
  var results = databaseConnection.query(
    `SELECT userName,passwordHash,passwordSalt FROM users WHERE userID = '${req.params.userId}' LIMIT 1`
  )[0];
  var passwordHash = results.passwordHash.toString();
  var passwordSalt = results.passwordSalt.toString();
  if (results.length != 0) {
    if (
      PWDTool.validatePassword(req.body.oldPassword, passwordHash, passwordSalt)
    ) {
