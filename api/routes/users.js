...
router.post('/:userId/forgetPassword', function(req, res) {
  var userData = databaseConnection.query(`SELECT userName,emailAddress FROM users WHERE userID = '${req.params.userId}' LIMIT 1`)[0]
  if(userData.length != 0){
    if(!PWDTool.isPendingPasswordReset(userData.userName)){
      ...
      databaseConnection.query(`INSERT INTO vulnwebsitedb.forgetPassword(userName,pincodeHash,pincodeSalt) VALUES ('${userData.userName}','${pincodeHash}','${pincodeSalt}')`)
      res.status(200).send(`reset password pending for user ${userData.userName}`)
      mailTansporter.notify(`${userData.emailAddress}`,`your pincode is: ${pincode}`,"VulnWebsite - Pincode for reset password")
    }else{
...
router.post('/:userId/changeForgottenPassword', function(req, res) {
  var userData = databaseConnection.query(`SELECT userName FROM users WHERE userID = '${req.params.userId}' LIMIT 1`)
  if(userData.length != 0){
    var username = userData[0].userName
    if(req.body.password && req.body.pincode){ //validation of params:
      if(PWDTool.isPendingPasswordReset(username)){
        var pendingData = databaseConnection.query(`SELECT pincodeHash,pincodeSalt FROM vulnwebsitedb.forgetPassword WHERE userName = '${username}'`)[0]
        if(PWDTool.validatePassword(req.body.pincode,pendingData.pincodeHash,pendingData.pincodeSalt)){ //validating pincode:
          if(!PWDTool.isComplexed(req.body.password)){ //checking password complexity:
            res.status(400).send(`not complexed`)
          }else if(!PWDTool.isPasswordUsed(username,req.body.password)){ //checking password history
            res.status(400).send(`password been used already. password retention set to ${PWD_HISTORY_CONFIG.history}`)
          }else{
      ...
      res.status(400).send("One or more parameters are not provided. Required parameters:'password','pincode'");
    }
  }else{
    res.status(404).send(`user not found`);  
  }
